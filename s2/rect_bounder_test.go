...
func TestRectBounderExpandForSubregions(t *testing.T) {
	...
	rectTests := []struct {
		...
		{1.5, -math.Pi / 2, 1.5, math.Pi/2 - 2e-16, Rect{Lat: r1.Interval{Lo: 1.5, Hi: 1.5}, Lng: s1.FullInterval()}},
		{1.5, -math.Pi / 2, 1.5, math.Pi/2 - 7e-16, Rect{Lat: r1.Interval{Lo: 1.5, Hi: 1.5}, Lng: s1.Interval{Lo: -math.Pi / 2, Hi: math.Pi/2 - 7e-16}}},
		// Check for cases where the bound is expanded to include one of the poles
		{-math.Pi/2 + 1e-15, 0, -math.Pi/2 + 1e-15, 0, Rect{Lat: r1.Interval{Lo: -math.Pi / 2, Hi: -math.Pi/2 + 1e-15}, Lng: s1.FullInterval()}},
		{math.Pi/2 - 1e-15, 0, math.Pi/2 - 1e-15, 0, Rect{Lat: r1.Interval{Lo: math.Pi/2 - 1e-15, Hi: math.Pi / 2}, Lng: s1.FullInterval()}},
	}
