...
func TestRectArea(t *testing.T) {
	tests := []struct {
		...
		{Rect{}, 0},
		{FullRect(), 4 * math.Pi},
		{Rect{Lat: r1.Interval{Lo: 0, Hi: math.Pi / 2}, Lng: s1.Interval{Lo: 0, Hi: math.Pi / 2}}, math.Pi / 2},
	}
	for _, test := range tests {
...
func TestRectVertex(t *testing.T) {
	r1 := Rect{Lat: r1.Interval{Lo: 0, Hi: math.Pi / 2}, Lng: s1.IntervalFromEndpoints(-math.Pi, 0)}
	tests := []struct {
		r    Rect
		i    int
		want LatLng
...
func TestRectVertexCCWOrder(t *testing.T) {
	for i := 0; i < 4; i++ {
		...
		r := Rect{
			Lat: r1.Interval{Lo: lat, Hi: lat + math.Pi/4},
			Lng: s1.Interval{
...
func testRectCentroidSplitting(t *testing.T, r Rect, leftSplits int) {
	...
	if oneIn(2) {
		...
		child0 = Rect{Lat: r1.Interval{Lo: r.Lat.Lo, Hi: lat}, Lng: r.Lng}
		child1 = Rect{Lat: r1.Interval{Lo: lat, Hi: r.Lat.Hi}, Lng: r.Lng}
	} else {
		lng := randomUniformFloat64(r.Lng.Lo, r.Lng.Hi)
		child0 = Rect{Lat: r.Lat, Lng: s1.Interval{Lo: r.Lng.Lo, Hi: lng}}
		child1 = Rect{Lat: r.Lat, Lng: s1.Interval{Lo: lng, Hi: r.Lng.Hi}}
	}
...
func TestRectCentroidFullRange(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		r := Rect{Lat: r1.Interval{Lo: lat1,Hi: lat2}, Lng: s1.FullInterval()}
		centroid := r.Centroid()
	...
	for i := 0; i < 100; i++ {
		...
		r := Rect{Lat: r1.Interval{Lo: -math.Pi / 2, Hi: math.Pi / 2}, Lng: s1.Interval{Lo: lat1, Hi: lat2}}
		centroid := r.Centroid()
	...
	// line of longitude.
	testRectCentroidSplitting(t, Rect{Lat: r1.Interval{Lo: -math.Pi / 2, Hi: math.Pi / 2}, Lng: s1.Interval{Lo: -math.Pi, Hi: math.Pi}}, 10)
}
